...
static void
man_provider_allow_fingerprint(struct management *man,
                               const char *credential_fingerprint,
                               const char *reason)
{
    if (man->persist.callback.provider_allow_fingerprint)
    {
        const bool status =
            (*man->persist.callback.provider_allow_fingerprint)(
                man->persist.callback.arg, credential_fingerprint, reason);
        if (status)
        {
            msg(M_CLIENT,
                "SUCCESS: provider-allow-fingerprint command succeeded");
        }
        else
        {
            msg(M_CLIENT, "ERROR: provider-allow-fingerprint command failed");
        }
    }
    else
    {
        man_command_unsupported("provider-allow-fingerprint");
    }
}
                     ...
                     const int nparms)
{
    ...
    else if (streq(p[0], "provider-allow-fingerprint"))
    {
        if (man_need(man, p, 1, MN_AT_LEAST))
        {
            man_provider_allow_fingerprint(man, p[1], p[2]);
        }
    }
