    ...
    struct provider_helper_ikev2_eap_tls_fragment *fragment)
{
    ...
    if (!body || body_len < PROVIDER_HELPER_EAP_TLS_HEADER_SIZE || !config)
    {
        return PROVIDER_HELPER_IKEV2_PARSE_BAD_PAYLOAD_LENGTH;
    }

    const uint8_t flags = body[5];
    if ((flags & PROVIDER_HELPER_EAP_TLS_FLAG_START)
        || (flags & ~PROVIDER_HELPER_EAP_TLS_FLAGS_ALLOWED))
    {
        return PROVIDER_HELPER_IKEV2_PARSE_UNEXPECTED_PAYLOAD;
    }

    size_t fragment_offset = PROVIDER_HELPER_EAP_TLS_HEADER_SIZE;
    uint32_t tls_message_len = 0;
    const bool length_included =
        (flags & PROVIDER_HELPER_EAP_TLS_FLAG_LENGTH_INCLUDED) != 0;
    const bool more_fragments =
        (flags & PROVIDER_HELPER_EAP_TLS_FLAG_MORE_FRAGMENTS) != 0;
    if (length_included)
    {
        ...
        fragment_offset += PROVIDER_HELPER_EAP_TLS_LENGTH_SIZE;
    }
...
provider_helper_ikev2_validate_eap_payload(
    ...
    const struct provider_helper_runtime_config *config)
{
    if (!body || body_len < PROVIDER_HELPER_EAP_HEADER_SIZE)
    {
        return PROVIDER_HELPER_IKEV2_PARSE_BAD_PAYLOAD_LENGTH;
    }
    ...
    switch (code)
    {
        case PROVIDER_HELPER_EAP_CODE_RESPONSE:
            if (body_len < PROVIDER_HELPER_EAP_TYPE_HEADER_SIZE)
            {
                return PROVIDER_HELPER_IKEV2_PARSE_BAD_PAYLOAD_LENGTH;
            }
        ...
        case PROVIDER_HELPER_EAP_CODE_FAILURE:
            return PROVIDER_HELPER_IKEV2_PARSE_UNEXPECTED_PAYLOAD;
